...
import {
  readCollaborators,
  readCollaboratorsByName,
  addCollaborator,
  editCollaborator,
  removeCollaborator,
  readCollaboratorsById,
} from '../data/collaborators.data';
...
const getCollaboratorsByName = (
  collaboratorName: string
): Promise<ServiceLayerResponse<Collaborator>> => {
  return new Promise((res, rej) => {
    readCollaboratorsByName(collaboratorName)
...
const getCollaboratorById = (
  id: string
): Promise<ServiceLayerResponse<Collaborator>> => {
  return new Promise((res, rej) => {
    readCollaboratorsById(id)
      .then((dataLayerResponse) => {
        if ((dataLayerResponse as Collaborator[]).length === 0) {
...
const putCollaborator = (
  id: string,
  body: Collaborator
): Promise<ServiceLayerResponse<Collaborator>> => {
  return new Promise((res, rej) => {
    editCollaborator(id, body)
      .then((dataLayerResponse) => {
        if (dataLayerResponse === 200) {
          res({
            code: 200,
            message: 'Collaborator updated successfully' as string,
          });
        }
      })
      .catch((error) => {
